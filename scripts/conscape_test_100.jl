...
for k in [2]
    ...
    coarse_target_qualities = ConScape.coarse_graining(g, 500)
    g = ConScape.Grid(size(hab_qual)...,
        affinities=adjacency_matrix,
        source_qualities=hab_qual,
    ...
    savefig("habitat_quality_$(k)_$(RESOLUTION).png")
    ...
    savefig("functional_habitat_$(k)_$(RESOLUTION).png")
    ...
    distance_transformation=x -> exp(-x/75))
    ConScape.heatmap(kbetw, yflip=true, title="Betweenness, $(LEGEND[k])", titlefontsize=8)
    savefig("flow_movement_$(k)_$(RESOLUTION).png")
end