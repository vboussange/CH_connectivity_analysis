...
WIP, we should modify it to implemet omniscape algorithm.
"""
import jax
import numpy as np
from jax import lax
...
def import_raster_and_info(species_name):
    path = Path("output") / species_name / "currmap_block_size_9.tif"
    resistance = load_raster(path, scale=False)
    resistance = jnp.array(resistance, dtype="bfloat16")
...
if __name__ == "__main__":
    ...
    permeability, info = import_raster_and_info(species_name)
    window_size = 9
    buffer_size = 5
