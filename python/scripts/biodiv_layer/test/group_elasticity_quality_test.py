...
def qKqT(hab_qual, activities, distance, D):
    ...
    x_core_window, y_core_window = jnp.meshgrid(jnp.arange(window_op.buffer_size, 
                                                ...
                                                jnp.arange(window_op.buffer_size, 
                                                            window_op.window_size+window_op.buffer_size))
    core_window_indices = grid.coord_to_active_vertex_index(x_core_window, y_core_window)
    ...
    K = jnp.exp(-dist) # calculating proximity matrix
    
    # qKqT = jnp.sum(core_window_qual**2)
    qKqT = (K @ q)
    # qKqT = hab_qual[window_center[0][1], window_center[0][1]]
    # qKqT = jnp.sum(core_window_qual)
    return qKqT

qKqT_grad_q = eqx.filter_jit(eqx.filter_grad(qKqT))
qKqT_grad_q_vmap = eqx.filter_vmap(qKqT_grad_q, in_axes=(0, 0, None, None))
...
if __name__ == "__main__":
    ...
    # # TODO: test to remove
    GROUP_INFO = {"Birds": EuclideanDistance()}
    for group in GROUP_INFO:
        ...
        # TEST:
        quality = quality[1500:1600, 1000:1100] # flip the raster        
        ...
        for (xy_batch, permeability_batch) in tqdm(batch_op.lazy_iterator(quality_padded), desc="Batch progress", total=batch_op.nb_steps):
            if not jnp.all(jnp.isnan(permeability_batch)):
                ...
                res = batch_run_calculation(batch_op, window_op, xy, qKqT_grad_q_vmap, hab_qual, activities, distance, D)
        ...
        plt.imshow(output[buffer_size+1:-buffer_size+1,buffer_size+1:-buffer_size+1])
        plt.colorbar()
        # plt.imshow(quality)